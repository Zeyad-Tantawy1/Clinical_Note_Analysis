...
class AdminDashboard
{
    ...
    {
        $sql = "SELECT COUNT(*) AS row_count FROM topics";  // Changed from 'note' to 'topics'
        $result = $this->db->query($sql);
    ...
    {
        $today = date('Y-m-d');
        $sql = "SELECT COUNT(*) AS today_count FROM topics WHERE DATE(date) = '$today'";  // Changed from 'note' to 'topics'
        $result = $this->db->query($sql);
        $row = $this->db->fetchAll($result)[0];
        $this->db->closeResult($result);
    ...
    {
        $sql = "SELECT DATE(date) as day, COUNT(*) as count FROM topics WHERE date BETWEEN '$startDate' AND '$endDate' GROUP BY DATE(date)";  // Changed from 'note' to 'topics'
        $result = $this->db->query($sql);
    ...
    {
        $sql = "SELECT status, COUNT(*) as count FROM topics GROUP BY status";  // Changed from 'note' to 'topics'
        $result = $this->db->query($sql);
